...
const InternalTab = ({ surveyData, setSurveyData, hideInputs = false }) => {
  const { t } = useTranslation();

  // When hideInputs is true, use values from autoDeck (shared input from DeckTab)
  ...
  // Get values either from local state or autoDeck
  const getValues = () => {
    if (hideInputs && surveyData.autoDeck) {
      return {
        leaderVal: parseInt(surveyData.autoDeck.skillLeader || '120') || 0,
        member2Val: parseInt(surveyData.autoDeck.skillMember2 || '100') || 0,
        member3Val: parseInt(surveyData.autoDeck.skillMember3 || '100') || 0,
        member4Val: parseInt(surveyData.autoDeck.skillMember4 || '100') || 0,
        member5Val: parseInt(surveyData.autoDeck.skillMember5 || '100') || 0,
      };
    }
    return {
      leaderVal: parseInt(leader || '120') || 0,
      member2Val: parseInt(member2 || '100') || 0,
      member3Val: parseInt(member3 || '100') || 0,
      member4Val: parseInt(member4 || '100') || 0,
      member5Val: parseInt(member5 || '100') || 0,
    };
  };
  ...
  useEffect(() => {
    ...
    const { leaderVal, member2Val, member3Val, member4Val, member5Val } = getValues();
  ...
  }, [leader, member2, member3, member4, member5, hideInputs, surveyData.autoDeck]);

  return (
    <div id="internal-tab-content" className="p-4 space-y-4">
      {/* Input Section - Hidden when hideInputs is true */}
      {!hideInputs && (
        <InputTableWrapper>
          <InputRow
